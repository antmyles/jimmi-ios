...
const SESSION_NAME_FALLBACK = "Manus user";
...
class OAuthService {
  ...
  private decodeState(state: string): string {
    ...
    try {
      ...
      // Older login links encoded the redirect URI directly as state.
    }
    return decoded;
  }
    ...
    return this.signSession(
      {
        openId,
        appId: ENV.appId,
        name: options.name?.trim() || SESSION_NAME_FALLBACK,
      },
      options
    );
  }
  ...
  async verifySession(
    ...
    try {
      ...
      return {
        openId,
        appId,
        name: isNonEmptyString(name) ? name : SESSION_NAME_FALLBACK,
      };
    } catch (error) {
